...
class StarlarkSandboxTest {
  ...
  void executesSimpleFireAndForgetRule() throws Exception {
    ...
    StarlarkSandbox sandbox = new StarlarkSandbox();
  ...
  void maxStepsEnforced() throws Exception {
    ...
    StarlarkSandbox sandbox = new StarlarkSandbox(100);
    StarlarkSandbox.ExecutionResult result =
  ...
  void effectWithFullChain() throws Exception {
    ...
    assertThat(result.success()).isTrue();
    assertThat(result.effects()).hasSize(2);
    EffectRecord fraud = result.effects().get(0);
